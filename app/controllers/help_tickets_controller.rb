class HelpTicketsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @help_ticket.save
        format.html { redirect_to @help_ticket, notice: 'Someone has been notified.' }
  ...
  private
    ...
    def set_help_ticket
      @help_ticket = HelpTicket.find_by(id: params[:id], user_id:current_user.id)
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def help_ticket_params
      params.require(:help_ticket).permit(:user_id)
    end
end
